...
import numpy as np
import torch
from sklearn.metrics import f1_score, roc_auc_score, hamming_loss, jaccard_score, precision_score, recall_score, \
...
def evaluate(data_loader, model, device):
    ...
    print("Prob row sum (should be ~1):", np.sum(y_probs, axis=1)[:10])
    ...
    ap = average_precision_score(np.eye(y_probs.shape[1])[y_true], y_probs, average='macro')
