...
from sklearn.metrics import (
    ...
    hamming_loss, jaccard_score, recall_score, precision_score, cohen_kappa_score, balanced_accuracy_score,
    confusion_matrix
)
...
def evaluate(data_loader, model, device, args, epoch, mode, num_class, log_writer):
    ...
    if log_writer:
        for metric_name, value in zip(
                ['balanced_accuracy', 'f1', 'roc_auc', 'hamming', 'jaccard', 'precision', 'recall', 'average_precision',
                 'kappa', 'score'],
    ...
    print(
        f'Balanced Accuracy: {accuracy:.4f}, F1 Score: {f1:.4f}, ROC AUC: {roc_auc:.4f}, Hamming Loss: {hamming:.4f},\n'
    ...
    # === PER-CLASS METRICS ===
    conf_matrix = confusion_matrix(true_labels, pred_labels, labels=list(range(num_class)))
    ...
    per_class_recall = TP / (TP + FN + epsilon)  # Sensitivity
    per_class_specificity = TN / (TN + FP + epsilon)
    per_class_balanced = (per_class_recall + per_class_specificity) / 2

    # Zapisz per-class metryki
    per_class_metrics_path = os.path.join(args.output_dir, args.task, f'per_class_metrics_{mode}.csv')
    with open(per_class_metrics_path, 'w', newline='', encoding='utf8') as cf:
        writer = csv.writer(cf)
        writer.writerow(['class', 'precision', 'recall (sensitivity)', 'specificity', 'balanced_accuracy'])
